...
export const LoginForm = () => {
  ...
  const urlError = searchParams.get("error") === "OAuthAccountNotLinked"
    ? "Email já associado a uma conta!"
    : "";
    ...
    <CardWrapper headerLabel="Bem-vindo de volta!" backButtonLabel="Não tem uma conta?" backButtonHref="/auth/register" showSocial>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <>
                ...
                <FormField control={form.control} name="password" render={({ field }) => <FormItem>
                  ...
                  <Button variant={"link"} size={"sm"} asChild className="px-0 font-normal">
                    <Link href={"/auth/reset"}>Esqueceu a senha?
                    </Link>
                  </Button>
                  <FormMessage />
                </FormItem>} />
          ...
          <Button type="submit" disabled={isPending} className="w-full">{showTwoFactor ? "Confirmar" : "Entrar"}</Button>
        </form>
      </Form>
    </CardWrapper>
  );
